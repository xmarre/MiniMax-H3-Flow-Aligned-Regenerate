...
Resolution-shift-only validation is now the next active feature path. E leaves Continuum at the
ordinary MP-driven base resolution and keeps the real **MiniMax H3 Latent Upscaler + Refine (3D)**
enabled at scale 1.20. A geometry-only **Refine Target Geometry** helper reads the same MP width/height
in parallel and mirrors the refine node's exact target-size calculation only so Resolution-Aware
Sigmas knows the downstream refine canvas. Its outputs are never fed into Continuum. The sigma remap
is inserted only on the refine node's SIGMAS path. For the current MP base 736x768, the helper resolves
the refine target to 896x928; source width/height stay at 0/0 so Resolution-Aware Sigmas auto-derives
the aspect-matched H3-Base reference 768x800. The SD3-derived factor is composed with H3's native shift
rather than replacing it. The safe default
...
| MiniMax H3 Refine Target Geometry | Geometry-only mirror of downstream learned-refine target sizing; supplies target metadata to the refine SIGMAS experiment only | Experimental |
...
`target_width/target_height`. Continuum keeps its ordinary MP width/height unchanged. The actual
Latent Upscaler + Refine node remains enabled and receives the remapped SIGMAS. The current MP base
736x768 therefore resolves to refine target 896x928, which auto-resolves the H3-native reference to
768x800.
