...
def test_candidate_numerical_policy_identity_is_stable_across_stage_owners():
    plan = _plan()
    first, first_policy = partitioned_positions_with_audio_policy(
        _Native,
        plan,
        _layout(),
        PARTITIONED_AUDIO_POSITION_DOMAIN_SOURCE,
        stage_owner_generation=101,
    )
    second, second_policy = partitioned_positions_with_audio_policy(
        _Native,
        plan,
        _layout(),
        PARTITIONED_AUDIO_POSITION_DOMAIN_SOURCE,
        stage_owner_generation=202,
    )
    assert torch.equal(first, second)
    assert first_policy.stage_owner_generation != second_policy.stage_owner_generation
    assert first_policy.signature == second_policy.signature
...
def test_tensor_sha256_accepts_bfloat16_without_dtype_conversion():
