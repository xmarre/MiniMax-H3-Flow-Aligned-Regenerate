...
def test_source_carrier_changes_only_target_audio_spatial_coordinates():
    ...
    assert policy.target_audio_w == (-8.0, 8.0)
    assert policy.source_audio_w == (-4.0, 4.0)
...
def test_candidate_policy_and_positions_are_stage_owned_and_immutable():
    ...
    drifted.position_ids[audio_start, 0] += 1.0
...
def test_candidate_execution_gate_fails_closed_then_accepts_actual_block_zero():
    ...
    with pytest.raises(RuntimeError, match="no actual low/probe"):
        _verify_audio_position_domain_diagnostic(
            metrics, PARTITIONED_AUDIO_POSITION_DOMAIN_SOURCE, calls_before=0
        )
...
def test_candidate_contract_requires_heterogeneous_context_and_normal_vdn():
    # This mirrors the scheduler preflight invariant without executing a sampler.
    def validate(prefix_context, vdn_mode):
        if prefix_context != PARTITIONED_PREFIX_TRANSFORMER_CONTEXT_EXACT:
            raise PartitionedPreflightUnsupported("exact_target_partitioned")
        if vdn_mode != PARTITIONED_VDN_LINEAR_DIAGNOSTIC_NORMAL:
            raise PartitionedPreflightUnsupported("vdn_linear_diagnostic='normal'")

    validate(PARTITIONED_PREFIX_TRANSFORMER_CONTEXT_EXACT, PARTITIONED_VDN_LINEAR_DIAGNOSTIC_NORMAL)
    with pytest.raises(PartitionedPreflightUnsupported):
        validate(PARTITIONED_PREFIX_TRANSFORMER_CONTEXT_SOURCE, PARTITIONED_VDN_LINEAR_DIAGNOSTIC_NORMAL)
    with pytest.raises(PartitionedPreflightUnsupported):
        validate(PARTITIONED_PREFIX_TRANSFORMER_CONTEXT_EXACT, PARTITIONED_VDN_LINEAR_DIAGNOSTIC_BYPASS)
