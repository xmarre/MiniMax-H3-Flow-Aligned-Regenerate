...
class H3PartitionedExactPrefixDiagnosticHandoff(H3PartitionedExactPrefixHandoff):
    ...
    def patch(
        ...
        if vdn_linear_diagnostic != PARTITIONED_VDN_LINEAR_DIAGNOSTIC_NORMAL:
            raise RuntimeError("PR fast exact-heterogeneous candidate requires vdn_linear_diagnostic='normal'")
        if audio_position_domain != PARTITIONED_AUDIO_POSITION_DOMAIN_SOURCE:
            raise RuntimeError("PR fast exact-heterogeneous candidate requires audio_position_domain='source_carrier'")
