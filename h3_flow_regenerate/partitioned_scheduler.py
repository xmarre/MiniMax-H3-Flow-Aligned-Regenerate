...
def run_partitioned_progressive(
    ...
    audio_guided_overlap_mode, _audio_mode_source = resolve_partitioned_audio_guided_overlap_mode(
    ...
    try:
        ...
        if audio_handoff_source == PARTITIONED_AUDIO_HANDOFF_SOURCE_SHADOW:
            ...
            with _source_uniform_audio_shadow_controls(transformer):
                ...
                transformer[PROBE_CONTEXT_KEY] = {
                    "outer_step": index,
                    "audio_handoff_shadow": True,
                }
                try:
                    _reset_guider_conds(guider, template=conditioning_template)
                    sampler_invocation_count += 1
                    ...
                    binding.metrics.increment("progressive_sampler_invocations")
                    binding.metrics.increment("progressive_history_boundaries")
                    binding.metrics.increment("partitioned_audio_handoff_shadow_probe_invocations")
                    with (
                        _flow_stage_contract(guider, "probe"),
                        _high_stage_contract(guider),
                        _partitioned_stage_contract(guider, stage_plan, binding.metrics),
                    ):
                        shadow_source_x0 = executor(
                            shadow_probe_noise,
                            low_latent_image,
                            _make_probe_sampler(sampler),
                            sigmas[index : index + 1],
            ...
            shadow_audio_timestep_calls = int(
                binding.metrics.counters.get("partitioned_audio_model_timestep_override_calls", 0)
            ...
            shadow_source_x0 = _process_latent_in(base_model, shadow_source_x0, source_shapes)
            _shadow_clean_video, shadow_clean_audio = unpack_streams(shadow_source_x0, source_shapes)
            shadow_clean_audio = shadow_clean_audio.detach().clone()
            binding.metrics.increment("partitioned_audio_handoff_shadow_probe_nfe")
            binding.metrics.event(
        ...
        audio_reference_stage = "low_probe_clean"
        low_probe_clean_audio = clean_audio.detach().clone() if diagnostic_audio_control else None
        if audio_handoff_source == PARTITIONED_AUDIO_HANDOFF_SOURCE_SHADOW:
            if shadow_clean_audio is None:
                raise RuntimeError("source-uniform audio handoff shadow lost its clean probe audio")
            low_probe_clean_audio = shadow_clean_audio
            audio_reference_stage = "source_uniform_shadow_probe_clean"
        if diagnostic_audio_control:
            diagnostic_audio_packed = source_x0
            if audio_handoff_source == PARTITIONED_AUDIO_HANDOFF_SOURCE_SHADOW:
                diagnostic_audio_packed = pack_streams((clean_video, low_probe_clean_audio))[0]
            low_probe_audio_report = measure_audio_latent_boundary(
                diagnostic_audio_packed,
                source_shapes,
                low_mask,
                windows=(4, 20),
            )
        ...
        if diagnostic_audio_control:
            ...
            binding.metrics.event(
                ...
                reference_stage=audio_reference_stage,
        ...
        if audio_handoff_source == PARTITIONED_AUDIO_HANDOFF_SOURCE_SHADOW:
            binding.metrics.event(
                ...
                final_exact_video_prefix=True,
                final_exact_audio_prefix=True,
                main_video_path_preserved=True,
