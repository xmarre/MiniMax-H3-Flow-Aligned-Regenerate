...
def _transport_video_state(
    ...
    *,
    ...
    # Preserve the source path/noise endpoint at the same flow coordinate. This
    # differs from PR #39's invalid full velocity re-anchor by weighting the
    # learned clean-anchor replacement with the flow clean coefficient.
    ...
    # Quantify exactly how far the failed PR #39 construction would overshoot
    # this endpoint-preserving state. Algebraically the difference is
    # sigma * (C_target - U(C_source)).
    lifted_velocity = _resize_field(source_state_work - source_clean_work, target_h, target_w).to(
        dtype=compute_dtype
    )
...
def _run_progressive_transport_wrapper(
    ...
    try:
        ...
        if record.enabled and record.transport_calls != 1:
            raise RuntimeError(
                "same-sigma endpoint transport expected exactly one learned handoff; "
