...
seen by the high-stage H3 prediction. Native inpaint has already produced
...
_ORIGINAL_DISABLED_SUFFIX_REPRESENTATION_BRIDGE_METRICS = _runtime.disabled_suffix_representation_bridge_metrics
...
def _capture_candidate(
    ...
    if not bool(metrics.get("suffix_representation_bridge_tone_bias_accepted")):
        report["suffix_representation_bridge_high_prefix_proxy_reason"] = "tone_rebase_not_authorized"
        return None, report
    if bool(metrics.get("suffix_representation_bridge_geometry_accepted")):
        report["suffix_representation_bridge_high_prefix_proxy_reason"] = "geometry_rebase_would_confound_proxy"
        return None, report
    geometry_reason = str(metrics.get("suffix_representation_bridge_geometry_reason", ""))
    if geometry_reason != "native_boundary_replacement_geometry_consistent":
        report["suffix_representation_bridge_high_prefix_proxy_reason"] = "clean_geometry_not_proven_consistent"
        return None, report

    prefix_t = int(exact_prefix.shape[2])
    if prefix_t <= 0 or prefix_t >= int(learned_clean_video.shape[2]):
        raise ValueError("PR32 prefix proxy requires a non-empty learned prefix and suffix")
    ...
    if deviation_rms > tolerance:
        report.update(
            suffix_representation_bridge_high_prefix_proxy_reason="authorized_bridge_not_pure_tone",
    ...
    exact = exact_prefix.detach().to(device=learned_prefix.device, dtype=torch.float32)
    ...
    report.update(
        ...
        suffix_representation_bridge_high_prefix_proxy_reason="tone_aligned_learned_prefix_ready",
...
def _apply_suffix_representation_bridge_capture(
    ...
    *,
    ...
    # Clear first so an exception or ineligible new run cannot reuse a prior GPU payload.
    ...
    payload, report = _capture_candidate(learned_clean_video, exact_prefix, corrected, metrics)
    _payload_var.set(payload)
    metrics.update(report)
    return corrected, metrics


def _disabled_suffix_representation_bridge_metrics_capture(*, prefix_t: int, requested: bool = False):
    _payload_var.set(None)
    metrics = dict(
    ...
    metrics.update(
        suffix_representation_bridge_high_prefix_proxy_mode=_MODE,
        suffix_representation_bridge_high_prefix_proxy_ready=False,
        suffix_representation_bridge_high_prefix_proxy_reason="representation_bridge_disabled",
        suffix_representation_bridge_high_prefix_proxy_clean_delta_rms=0.0,
        suffix_representation_bridge_high_prefix_proxy_bias_deviation_rms=0.0,
    )
    return metrics
...
def _native_visual_cond_timestep() -> float:
    ...
    if not math.isfinite(value) or not 0.0 <= value <= 1.0:
        raise RuntimeError("native MiniMax H3 visual conditioning timestep is invalid")
    return value
...
def _proxy_packed_model_input(
    ...
    *,
    ...
    shapes = [tuple(int(value) for value in shape) for shape in contract["shapes"]]
    if len(shapes) != 2:
        raise RuntimeError("PR32 prefix proxy requires packed MiniMax H3 video/audio shapes")
    video, audio = unpack_streams(packed, shapes)
    if tuple(video.shape) != payload.video_shape:
        raise RuntimeError("PR32 prefix proxy payload geometry does not match the active high-stage model input")
    ...
    if not math.isfinite(aug) or not 0.0 <= aug <= 1.0:
        raise ValueError("PR32 prefix proxy visual conditioning timestep must be in [0, 1]")
    ...
    proxied_video[:, :, :prefix_t] = (proxied_video[:, :, :prefix_t].float() + delta).to(proxied_video.dtype)
    ...
    prefix_delta = proxied_video[:, :, :prefix_t].float() - video[:, :, :prefix_t].float()
    suffix_delta = proxied_video[:, :, prefix_t:].float() - video[:, :, prefix_t:].float()
    ...
    if telemetry["prefix_proxy_suffix_delta_rms"] != 0.0 or telemetry["prefix_proxy_audio_delta_rms"] != 0.0:
        raise RuntimeError("PR32 prefix proxy modified generated suffix or audio input")
    return proxied, telemetry


class _ModelOnlyPrefixProxyExecutor:
    def __init__(self, executor: Any, contract: dict[str, Any], payload: _PrefixProxyPayload) -> None:
...
def flow_predict_wrapper_with_prefix_proxy(executor, x, timestep, model_options=None, seed=None):
    """Proxy only the inner H3 input while Flow keeps the real high-stage state."""
    ...
    result = _ORIGINAL_FLOW_PREDICT_WRAPPER(adapted, x, timestep, model_options, seed)
    if adapted.telemetry is None or adapted.proxied_input is None:
        raise RuntimeError("PR32 high-stage prefix proxy was armed but did not reach the H3 executor")
...
def install() -> None:
    ...
    if getattr(_runtime.flow_predict_wrapper, "_h3_pr32_prefix_proxy", False):
        return
    _runtime.apply_suffix_representation_bridge = _apply_suffix_representation_bridge_capture
    _runtime.disabled_suffix_representation_bridge_metrics = _disabled_suffix_representation_bridge_metrics_capture
