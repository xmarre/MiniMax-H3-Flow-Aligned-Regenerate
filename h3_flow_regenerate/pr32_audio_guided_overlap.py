"""PR #32 generation-side audio boundary diagnostic.

00419 proves that the decoded/assembly click is fixed by the global 40-Hz
phase correction while the first newly generated audio latent still has a much
larger-than-local transition. This module provides one deliberately bounded
experiment: keep the exact video mask untouched, but replace the last few
*carried audio* mask ticks with a monotonic denoise-strength ramp during the
sampler lifetime. The caller keeps the original exact mask separately and
restores every originally protected value at the framework boundary.

This is diagnostic-only. It does not redefine Native Masked continuation and
is disabled unless H3_FLOW_PR32_AUDIO_GUIDED_OVERLAP_TICKS is explicitly set.
"""
...
AUDIO_GUIDED_OVERLAP_ENV = "H3_FLOW_PR32_AUDIO_GUIDED_OVERLAP_TICKS"
MAX_AUDIO_GUIDED_OVERLAP_TICKS = 16
_MASK_QUANTIZATION_LEVELS = 256.0


def configured_audio_guided_overlap_ticks() -> int:
    """Return the explicitly requested diagnostic width in 40-Hz audio ticks."""

    raw = str(os.environ.get(AUDIO_GUIDED_OVERLAP_ENV, "")).strip()
    if not raw:
        return 0
    try:
        ticks = int(raw)
...
def apply_audio_guided_overlap_mask(
    ...
    *,
    ...
    lifetime. The values are explicitly rounded upward onto Core MiniMax-H3's
    1/256 mask grid, matching ``MiniMaxH3._token_grid_masks`` rather than
    relying on an implicit downstream rounding pass.
    ...
    if latent_shapes is None or len(latent_shapes) != 2:
        raise ValueError("PR32 audio guided overlap requires native packed H3 video/audio shapes")
    if denoise_mask.ndim != 3 or denoise_mask.shape[1] != 1:
        raise ValueError("PR32 audio guided overlap requires packed Bx1xN denoise mask")
    ...
    if audio_mask.ndim != 4 or int(audio_mask.shape[2]) != 2:
        raise ValueError("PR32 audio guided overlap requires native BxCx2xT audio mask")
    ...
    if prefix <= 0:
        raise ValueError("PR32 audio guided overlap found protected audio without a leading exact prefix")
    if not bool(exact_one[prefix:].all().item()):
        raise ValueError(
            "PR32 audio guided overlap requires a contiguous exact audio prefix followed by a fully generated suffix"
        )
    if ticks >= prefix:
        raise ValueError(f"PR32 audio guided overlap width {ticks} must be smaller than exact audio prefix {prefix}")
