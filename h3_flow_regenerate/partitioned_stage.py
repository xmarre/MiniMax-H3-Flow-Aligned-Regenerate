...
class PartitionedPositionPolicy:
    ...
    def signature(self) -> tuple[object, ...]:
        # Numerical-history identity must be stable across equivalent low/probe
        # stage-owner reconstruction. The owner generation remains a telemetry
        # receipt on the policy record, but is not part of the numerical policy.
        return (
...
def _target_audio_range(layout) -> tuple[int, int]:
    matches = [(int(start), int(stop)) for start, stop, kind in layout.segments if kind == "audio"]
    if len(matches) != 1:
