"""Validation overlay that isolates the first high-stage Spectrum forecast.

00441 established that the learned target anchor and the first transported
guidance reference are clean, while the final high-stage pre-guidance prediction
is already corrupted. The accepted short schedule has three high-stage logical
calls: actual -> forecast -> actual. This diagnostic requests two exact
high-stage prefix calls through the existing ``h3_refinement`` Spectrum
contract. For that schedule the only high-stage forecast is therefore replaced
by an actual H3 evaluation.

The intervention is deliberately narrow: guided ``ProgressiveTargetInputConfig``
runs using learned_3d transfer and fallback exact-prefix mode. Probe and low
stages retain their existing contracts. This is diagnostic-only and must not be
promoted as a production schedule change without decoded-media evidence.
"""
...
def _high_stage_contract_isolation_wrapper(guider: Any):
    ...
    if previous is not None:
        ...
        if conflicts:
            raise RuntimeError(f"existing h3_refinement contract conflicts with high-stage forecast isolation: {conflicts}")
