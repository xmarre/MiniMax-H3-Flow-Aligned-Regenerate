...
def _validate_audio_position_policy(
    ...
    *,
    ...
    for event in transformer_events:
        ...
        signature = fields.get("audio_position_policy_signature")
        _require(bool(signature), "source-carrier candidate is missing its numerical-history policy signature")
        signatures.append(repr(signature))
    _require(
        len(set(signatures)) == 1,
        "source-carrier candidate position policy changed within the low/probe evidence window",
    )
