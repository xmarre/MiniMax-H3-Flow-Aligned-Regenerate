...
def _memory_preflight(device: torch.device) -> dict[str, Any]:
    ...
    try:
        ...
        if hasattr(os, "sysconf"):
            try:
                cpu_available = int(os.sysconf("SC_AVPHYS_PAGES")) * int(os.sysconf("SC_PAGE_SIZE"))
            except (OSError, ValueError):
                cpu_available = None
    if cpu_available is None:
        raise RuntimeError("first-high Sol-local E cannot prove the required 2 GiB host-memory headroom")
    report = {
        "required_free_bytes_each": _MIN_FREE_BYTES,
