...
pixels. The bridge uses independently corroborated source/target boundary
residuals to select candidate axes, then derives a per-axis temporal correction
policy from the genuine source-grid continuation. The authoritative prefix is
...
def _motion_model(video: torch.Tensor, prefix_t: int | None = None) -> dict:
    """Extrapolate the next natural per-frame transform from recent prefix motion."""
...
def _predict_motion_transform(motion: dict, offset: int) -> tuple[float, float, float, float] | None:
    """Predict a future natural transition transform at the given suffix offset."""
...
def _motion_residual_candidate(boundary: dict, motion: dict, hw) -> dict:
    ...
    deferred until cross-grid evidence is available.
    """
    report = {
        "accepted": False,
        "provisional": False,
...
def _project_source_transform(candidate: dict, source_hw, target_hw) -> tuple[float, float, float, float]:
    """Project source translation units onto the target grid while preserving scale."""
    sh, sw = source_hw
    th, tw = target_hw
...
def _corroborate_target_candidate(source_candidate: dict, target_candidate: dict, source_hw, target_hw) -> dict:
    """Authorize same-axis cross-grid evidence before temporal classification.
    ...
    axis_reasons = []
    for axis in range(4):
        shared = bool(source_axes[axis] and target_axes[axis])
        source_value = float(source_signed[axis])
        target_value = float(target_signed[axis])
        same_sign = shared and source_value * target_value > 0
        ratio = abs(target_value) / abs(source_value) if shared and abs(source_value) > 1e-12 else None
        ...
        domain_ok = shared and min(source_score, target_score) >= _MIN_DOMAIN_EVIDENCE
        combined = math.hypot(source_score, target_score) if shared else 0.0
        ...
        if not source_axes[axis]:
            axis_reason = "source_axis_not_provisional"
        elif not target_axes[axis]:
            axis_reason = "target_axis_not_provisional"
        elif not same_sign:
        ...
        shared_candidate.append(shared)
        sign_agreement.append(bool(same_sign))
        magnitude_ratio.append(ratio)
        combined_scores.append(combined)
        domain_floor_pass.append(bool(domain_ok))
        axis_authorized.append(authorized)
        axis_reasons.append(axis_reason)

    target_transform = target_candidate["transform"]
    applied = (
        target_transform[0] if axis_authorized[0] else 1.0,
        target_transform[1] if axis_authorized[1] else 1.0,
        target_transform[2] if axis_authorized[2] else 0.0,
        target_transform[3] if axis_authorized[3] else 0.0,
    )
    report.update(
        ...
        shared_candidate_axis=shared_candidate,
        sign_agreement=sign_agreement,
        magnitude_ratio=magnitude_ratio,
        axis_reason=axis_reasons,
        axis_applied=axis_authorized,
        axis_authorized=axis_authorized,
        transform=applied,
    )
    if not any(axis_authorized):
...
def _temporal_axis_state(
    ...
    *,
    ...
    """Classify one authorized axis and derive a measured monotonic envelope.

    Recovery is accepted only when the measured cumulative state reaches the
    estimator floor or crosses sign. Persistent state requires at least two
    usable follow-up transitions and must stay within the estimator's meaningful
    resolution of the initial boundary residual. Intermediate growth or reversal
    larger than that floor is treated as unsafe rather than extrapolated.
    """
    ...
    if recovery_index is not None:
        ...
        active_tokens = min(suffix_length, max((i + 1 for i, weight in enumerate(weights) if weight > 0.0), default=0))
        report.update(
            accepted=True,
            mode="recovering",
...
def _suffix_temporal_profile(video: torch.Tensor, prefix_t: int, motion: dict, candidate: dict, corroboration: dict) -> dict:
    ...
    report = {
        "accepted": False,
        "reason": "cross_grid_authorization_unavailable",
        "transitions": [],
    ...
    if not candidate.get("accepted") or not motion.get("accepted") or not any(authorized_axes):
        report["axis"] = [
            {
                "selected": bool(authorized_axes[axis]),
    ...
    for offset in range(min(PERSISTENCE_TRANSITIONS, available)):
        ...
        if item.get("aligned_error") is not None and math.isfinite(float(item["aligned_error"])):
            ...
            usable = item["aligned_error"] <= 0.30
            if usable:
                usable_count += 1
    ...
    axis_reports = []
    axis_accepted = [False] * 4
    axis_modes = ["inactive"] * 4
    suffix_length = int(video.shape[2]) - int(prefix_t)
    floors = candidate.get("axis_estimator_floor", [_SCALE_FLOOR, _SCALE_FLOOR, _TRANSLATION_FLOOR, _TRANSLATION_FLOOR])
    boundary_signed = candidate.get("raw_signed_residual", _signed_components(candidate["transform"]))
    for axis in range(4):
        if not authorized_axes[axis]:
            axis_reports.append(
                {
                    "selected": False,
                    "cross_grid_authorized": False,
                    "mode": "inactive",
                    "accepted": False,
                    "reason": "axis_not_cross_grid_authorized",
                }
            )
            continue
        ...
        axis_report.update(selected=True, cross_grid_authorized=True)
        ...
        axis_accepted[axis] = bool(axis_report["accepted"])
        axis_modes[axis] = axis_report["mode"]

    report.update(axis=axis_reports, axis_accepted=axis_accepted, axis_mode=axis_modes)
    if not any(axis_accepted):
        report["reason"] = "no_temporally_safe_axis"
        return report
    report.update(accepted=True, reason="temporally_safe_axes_classified")
    return report


def _axis_weight(axis_report: dict, token_index: int) -> float:
    """Return the correction weight for one temporal token from a compact policy."""
    if not axis_report.get("accepted"):
        return 0.0
    policy = axis_report.get("applied_envelope", {})
    if policy.get("kind") == "constant":
        return float(policy.get("value", 1.0)) if token_index < int(policy.get("active_tokens", 0)) else 0.0
    if policy.get("kind") == "measured_monotonic":
        weights = policy.get("weights", [])
        return float(weights[token_index]) if token_index < len(weights) else 0.0
    return 0.0


def _effective_transform(
    base_transform: tuple[float, ...], axis_reports: list[dict], token_index: int
) -> tuple[float, float, float, float]:
    """Interpolate scale in log space and translation linearly for one token."""
...
def _warp_video_suffix(
    video: torch.Tensor,
    prefix_t: int,
    base_transform: tuple[float, ...],
    temporal_profile: dict,
) -> tuple[torch.Tensor, dict]:
    """Apply token-varying transforms to only the active suffix prefix in one batch."""
    suffix = video[:, :, prefix_t:]
    suffix_t = int(suffix.shape[2])
    axis_reports = temporal_profile.get("axis", [])
    active_t = max((int(axis.get("active_tokens", 0)) for axis in axis_reports if axis.get("accepted")), default=0)
    active_t = min(active_t, suffix_t)
    metadata = {
    ...
    with torch.autocast(device_type=video.device.type, enabled=False):
        theta = torch.empty((active_t, 2, 3), device=video.device, dtype=torch.float32)
        theta[:, 0, 1] = 0.0
        theta[:, 1, 0] = 0.0
    ...
    corrected[:, :, prefix_t : prefix_t + active_t] = warped.reshape(b, active_t, c, h, w).permute(0, 2, 1, 3, 4)
    same_transform = all(transform == transforms[0] for transform in transforms[1:])
    compact_transforms = [transforms[0]] if same_transform else transforms
    metadata.update(
        effective_active_temporal_length=active_t,
        tokens_corrected=active_t,
        applied_transforms=compact_transforms,
...
def geometric_seam_bridge(learned, exact, *, source_hw, requested):
    """Correct cross-grid corroborated axes using a source-measured temporal state."""
    ...
    if source_valid:
        ...
        if source_candidate.get("accepted"):
            ...
            temporal_profile = _suffix_temporal_profile(source, p, source_motion, source_candidate, corroboration)
        low_report.update(
    ...
    temporally_safe_axes = temporal_profile.get("axis_accepted", [False] * 4)
    base_target = corroboration.get("transform", IDENTITY)
    final_transform = (
        base_target[0] if temporally_safe_axes[0] else 1.0,
        base_target[1] if temporally_safe_axes[1] else 1.0,
        base_target[2] if temporally_safe_axes[2] else 0.0,
        base_target[3] if temporally_safe_axes[3] else 0.0,
    )
    candidate_ready = bool(any(temporally_safe_axes))
    ...
    elif not corroboration.get("accepted"):
        reason = corroboration["reason"]
    elif not temporal_profile.get("accepted"):
        reason = temporal_profile["reason"]
    else:
        reason = "temporal_cross_grid_geometry_authorized"

    modes = temporal_profile.get("axis_mode", ["inactive"] * 4)
    report = {
        ...
        "motion_residual": source_candidate,
        "suffix_persistence": temporal_profile,
        "temporal_profile": temporal_profile,
        "cross_grid_corroboration": corroboration,
        "requested": bool(requested),
        "accepted": accepted,
        "reason": reason,
        "policy": "cross_grid_authorized_per_axis_temporal_residual_envelope",
        "persistent_bias_candidate": any(mode == "persistent" for mode in modes),
        "transient_bias_candidate": any(mode == "recovering" for mode in modes),
        "temporal_bias_candidate": candidate_ready,
        "corroborated_bias_candidate": bool(corroboration.get("accepted")),
        "base_target_transform": final_transform,
        "tokens_corrected": 0,
    ...
    if accepted:
        corrected, warp_report = _warp_video_suffix(learned, p, final_transform, temporal_profile)
        report.update(warp_report)
        if corrected is learned:
            report.update(accepted=False, reason="no_nonidentity_temporal_transform")
