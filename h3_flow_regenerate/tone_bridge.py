...
def apply_suffix_residual_transport(
    ...
    learned_boundary = upscaled_clean_video[:, :, prefix_t].float() - upscaled_clean_video[:, :, prefix_t - 1].float()
    ...
    if suffix_t > 1:
        learned_suffix_delta = (
            upscaled_clean_video[:, :, prefix_t + 1 :].float() - upscaled_clean_video[:, :, prefix_t:-1].float()
        )
        corrected_suffix_delta = corrected[:, :, prefix_t + 1 :].float() - corrected[:, :, prefix_t:-1].float()
...
def map_clean_bridge_to_conditional_state(
    ...
    *,
    ...
    if not torch.equal(clean_before[:, :, :prefix_t], clean_after[:, :, :prefix_t]):
        raise RuntimeError("conditional bridge mapping attempted to alter the authoritative prefix")
    if not torch.equal(
        clean_before[:, :, prefix_t + corrected_tokens :], clean_after[:, :, prefix_t + corrected_tokens :]
    ):
        raise RuntimeError("conditional bridge mapping attempted to alter values outside the corrected suffix range")
