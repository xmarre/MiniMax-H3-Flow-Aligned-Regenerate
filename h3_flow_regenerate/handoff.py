...
class ProgressiveTargetInputConfig:
    ...
    reducing only the early H3 transformer token stream.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.exact_prefix_mode not in {"fallback", "target_sparse_lifter"}:
            raise ValueError("exact_prefix_mode must be fallback or target_sparse_lifter")
        if self.min_high_steps < 1:
            raise ValueError("min_high_steps must be positive")
