...
def _validate_file_receipt(receipt: Any, *, label: str) -> None:
    if not isinstance(receipt, dict) or not _is_sha256(receipt.get("sha256")):
        raise ValueError(f"{label} is missing a canonical SHA-256 receipt")
    size = receipt.get("bytes")
    if type(size) is not int or size <= 0:
        raise ValueError(f"{label} is missing a positive byte count")
...
def validate_fixture_manifest(manifest: dict[str, Any]) -> str:
    ...
    for index, receipt in enumerate(references):
        _validate_file_receipt(receipt, label=f"reference[{index}]")

    assets = manifest.get("assets")
    if not isinstance(assets, dict):
        raise ValueError("exact-prefix fixture manifest is missing assets")
    for name in REQUIRED_FIXTURE_ASSETS:
        _validate_file_receipt(assets.get(name), label=f"fixture asset {name!r}")

    runtime = manifest.get("runtime")
    if not isinstance(runtime, dict):
        raise ValueError("exact-prefix fixture manifest is missing runtime identities")
    missing_runtime = [name for name in REQUIRED_RUNTIME_IDENTITIES if not str(runtime.get(name, "")).strip()]
    if missing_runtime:
        raise ValueError(f"exact-prefix fixture manifest is missing runtime identities: {missing_runtime!r}")
    if type(manifest.get("sampler_seed")) is not int:
        raise ValueError("exact-prefix fixture manifest is missing an integer sampler seed")
    wildcard_seed = manifest.get("wildcard_seed")
    if wildcard_seed is not None and type(wildcard_seed) is not int:
        raise ValueError("exact-prefix fixture wildcard seed must be an integer or null")

    archived = manifest.get("archived_evidence")
    if not isinstance(archived, dict):
        raise ValueError("exact-prefix fixture archived-evidence field must be a mapping")
    if archived:
        missing_archived = [name for name in ARCHIVED_EVIDENCE_SHA256 if name not in archived]
        if missing_archived:
            raise ValueError(f"exact-prefix fixture archived evidence is incomplete: {missing_archived!r}")
        for name, expected in ARCHIVED_EVIDENCE_SHA256.items():
            _validate_file_receipt(archived[name], label=f"archived evidence {name!r}")
            if archived[name]["sha256"] != expected:
                raise ValueError(f"archived evidence hash mismatch for {name!r}")
    return supplied
...
def validate_run_receipt(
    ...
    *,
    ...
    for name in REQUIRED_RUN_OUTPUTS:
        digest = outputs.get(f"{name}_sha256")
        size = outputs.get(f"{name}_bytes")
        if not _is_sha256(digest):
            raise ValueError(f"{label} run receipt is missing a canonical {name}_sha256")
        if type(size) is not int or size <= 0:
            raise ValueError(f"{label} run receipt is missing a positive {name}_bytes")
...
def validate_candidate_pair(
    ...
    required_equal_settings = ("sampler", "schedule", "handoff", "prompt")
    missing_equal = [name for name in required_equal_settings if name not in control_settings]
    if missing_equal:
        raise ValueError(f"matched pair is missing controlled settings: {missing_equal!r}")
