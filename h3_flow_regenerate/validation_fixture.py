...
def validate_candidate_pair(
    ...
    for label, receipt in (("control", control), ("candidate", candidate)):
        ...
        for name in ("log_sha256", "metrics_sha256", "mp4_sha256", "pre_patch_boundary_sha256"):
            value = str(outputs.get(name, ""))
            if len(value) != 64:
                raise ValueError(f"{label} run receipt is missing {name}")
