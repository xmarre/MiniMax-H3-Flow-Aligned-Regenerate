...
def _run_initial_transfer_diagnostic(
    ...
    *,
    ...
    if partitioned_progressive.transfer_mode != "learned_3d":
        raise RuntimeError(
            "bicubic initial-transfer diagnostic requires learned_3d as the production baseline"
        )
    ...
    if released_progressive.transfer_mode != "learned_3d":
        raise RuntimeError(
            "initial-transfer diagnostic expected the released progressive config to use learned_3d"
        )
    if (
        released_progressive.source_latent_h != partitioned_progressive.source_latent_h
        or released_progressive.source_latent_w != partitioned_progressive.source_latent_w
        or released_progressive.source_scale != partitioned_progressive.source_scale
        or released_progressive.handoff_coordinate != partitioned_progressive.handoff_coordinate
        or released_progressive.handoff_selection != partitioned_progressive.handoff_selection
    ):
        raise RuntimeError("initial-transfer diagnostic detected progressive geometry/config drift")
