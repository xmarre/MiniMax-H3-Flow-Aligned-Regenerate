...
v0.3.4 fixes the decode boundary exposed by the standalone Continuum learned-upscale/refine path and corrects the current performance documentation to keep native generation, Flow progressive sampling, and the separate two-sampler upscale/refine workflow distinct.
