...
A prior decoded-media smoke exposed an important ambiguity failure in repetitive texture, but that media run is now treated as confounded because the decode used the unintended TensorRT VAE `w4a16_awq` decoder. The matcher defect was still real and remains fixed. High
...
The repeated/patterned grass seen in that run can no longer be attributed to the temporal matcher because the unintended VAE decoder was an independent media-quality confound.
