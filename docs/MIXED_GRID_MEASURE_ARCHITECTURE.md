...
Likely files: `sol_h3/mixed_measure.py`, `runtime.py`, vendor `interface.py`, `preprocess.py`, `sm120/mainloop.py` and launch/cache plumbing. Existing SM120 routing already ORs an exact K-sink interval into the route mask and removes those blocks from the approximate branch. Extend its exact-score path with natural-log key bias, converted exactly once to the kernel's base-2/scaled-score representation **before running-max and exponentiation**. The current helper stores raw-dot-product maxima. Add `b/scale` to each valid exact raw score before the existing max/softmax helper, with a validated finite positive scale and FP32 arithmetic; the helper then applies `scale * log2(e)` once. This preserves its existing running-max units and avoids a broad softmax refactor. Preserve no-bias compilation/numerics.
...
Required kitchen extension: add optional keyword-only `key_bias` to chunked API, normalize to contiguous natural-log bias, convert to kernel log2 units once, propagate through nanobind/C++ producer launch and `sol_attn_producer.cu` into existing per-key `ksb` bias storage. The audited `sol_layout.cuh::quant_k_rows` stores `(key_scale, key_bias)` in `ksb`, with zero bias for live keys when the pointer is null and NEG for padding. Pass the correctly offset logical-key bias pointer into this helper; preserve its `perm_key` mapping and padding sentinel. K centering already subtracts kmean from K and requires no additional per-key compensation here. Do not route bias through kmean/vscale. Update bindings, fake/shape contracts where applicable, CUDA tests and HIP parity/capability declaration. Retain callable producer bootstrap and chunk alignment validation.
...
Upstream ComfyUI and kitchen changes are required for **native chunked BSA support**, but the Sol/dense path can be developed first. Do not emulate chunked BSA by materializing full QKV silently; that loses its memory contract. The audited core `h3_sparse_attention` returns `attn.out_proj(out)` directly, and `make_h3_block_patch` supplies that callable to the block; this bypasses `attn.forward`. VDN applies its learned gate inside its replacement `vdn_forward`. Therefore the bare core primitive does not itself preserve that gate. This is a source-level composition hazard, not a new attribution of the reported framing defect or a claim about uninspected production overlays. The mandatory bridge below must resolve it for the new mixed route; otherwise reject that combination.
...
For the chunked path, define a VDN-owned `vdn_h3_external_softmax_epilogue_v1` capability on the patched attention owner. Its `prepare(x, rope_freqs, options, block_index)` validates API-2 topology and returns a run-scoped epilogue bound to VDN state, branch/config and weight owner. Core BSA computes **unprojected** `[T,H,D]` softmax output, then invokes `epilogue.apply(softmax_out, x)`: VDN obtains weights through `state.weights_on`, applies its released sigmoid softmax gate when enabled, casts/flattens and calls the original `out_proj` exactly once. A block without a VDN branch uses native projection only. The epilogue must not invoke another attention call or a linear complement. Record the VDN owner/config digest in measure/history identity and a gate/projection completion field in the block receipt. Core recognizes this explicit capability rather than importing VDN internals or copying gate math. For non-mixed/high-grid calls, retain the existing path; this bridge is scoped to the validated external mixed contract. Add a numerical gate oracle using nontrivial gates and a projection call counter so a bypass cannot pass contract tests.
