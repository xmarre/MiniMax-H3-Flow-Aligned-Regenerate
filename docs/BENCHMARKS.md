...
Attach **MiniMax H3 Metrics JSON** to each experimental path. Its required **after** input is an
execution barrier: connect it to that path's final generated LATENT/IMAGE/AUDIO artifact so the JSON
cannot render before sampling/refinement has mutated the sink. In the two-pass path, route the
